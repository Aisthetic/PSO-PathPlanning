...
let croise_obstacle = fun seg obst ->
	(*print_s_lst lst_segments;*)
	let rec f_aux = fun lst -> (*On regarde si le segment croise un de ceux composant l'obstacle*)
		match lst with
			|[] -> false
			|[p] -> false
			|h1::h2::q when (croise_segment seg (h1,h2)) ->  true
			|h1::h2::q when not (croise_segment seg (h1,h2)) -> f_aux (h2::q)
	...
	for i=0 to (Array.length tab - 2) do
		...
		let rec aux1 = fun o ->
			match o with 
				|[] -> false
				|[p] -> false
				|h1::h2::q -> if (croise_segment (h1,h2) s) then true else aux1 (h2::q)
		in
		let rec aux = fun obst j ->
			match obst with 
				|[] -> true
				|h::q -> if (aux1 h) then false else (aux q (j+1))
		in 
		if not (aux obst 1) then test := false;
	done;
	!test
;;
...
(* let trajectoire_ok = fun traj lst_obst ->
	let rec ok_pour_un = fun traj obst ->
		match traj with
			|[] -> true
			|[p] -> true
			|h1::h2::q when (croise_obstacle (h1,h2) obst) -> false
			|h1::h2::q when not (croise_obstacle (h1,h2) obst) -> ok_pour_un q obst
			|_ -> failwith "Erreur ok_pour_un"
	in 
	let rec f_aux = fun lst_o ->
		match lst_o with
			|[] -> true
			|t::q when not (ok_pour_un traj t) -> false
			|t::q when (ok_pour_un traj t) -> f_aux q
			|_ -> failwith "Erreur trajectoire_ok"
	in (f_aux lst_obst);;

let trajectoire_ok2 = fun traj lst_obst ->
	let rec ok_pour_un = fun traj obst ->
		match traj with
			|[] -> true
			|[p] -> true
			...
			|_ -> failwith "Erreur ok_pour_un"
	in 
	let rec f_aux = fun lst_o i->
		match lst_o with
			|[] -> true
			|t::q when not (ok_pour_un traj t) -> false
			|t::q when (ok_pour_un traj t) -> f_aux q (i+1)
			|_ -> failwith "Erreur trajectoire_ok"
	in (f_aux lst_obst 1);;
 *)

(* let croise_obstacle2 = fun seg obst ->
	let lst_segments = segmente_obstacle obst in
	(*let a,b = seg in*)
	(*Printf.printf "Seg = [ (%f,%f) , (%f,%f) ]\n" a.x a.y b.x b.y;*) 
	(*print_string " + 1 \n";*)
	print_s_lst lst_segments;
	let rec f_aux = fun lst -> (*On regarde si le segment croise un de ceux composant l'obstacle*)
		match lst with
			|[] -> false
			|t::q -> if (croise_segment seg t) then true else f_aux q
	in f_aux lst_segments;; *)